 ...
 * work is allowed to run at all — and the one with no readable state anywhere (ADR-0003's Phase 4a
 * amendment). Named explicitly in the manifest's `<queries>` so it can be looked up at all.
 */
 ...
 * the exemption, and this app's core function is a rabbit's weight chart. So the app reads the
 * state, explains it, and opens the screen where the owner decides.
 *
...
/**
 * Opens Android's battery-optimisation list, where the owner can exempt this app themselves.
 *
 * The app-specific action (`ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`) is the one that needs the
 * restricted permission, so this is its unrestricted sibling: the whole list, with the owner one
 * search away from us. Best-effort in the same shape as `openSystemBackupSettings`, and for the same
 * reason — a dead button costs the owner more than one extra tap does.
 *
...
fun Context.openBatteryOptimisationSettings(): Boolean {
    ...
    for (action in actions) {
        ...
        // without it the owner cannot bring the app forward again while they are in there.
 ...
 * the setting has no public state, so the app cannot know afterwards what the owner did — and a
 * checkbox asking them to confirm would have the app repeating the owner's guess back to them as its
