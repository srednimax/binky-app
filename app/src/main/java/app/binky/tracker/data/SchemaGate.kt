...
/**
 * **What a launch should do about the database that is already on the phone** (ADR-0007, ADR-0023).
 *
 ...
 * The cost of the conflation was total: an owner updating across a schema bump met the refusal
 * screen, and the migration this project tests against three real archives never ran.
 */
enum class SchemaGate {
    /** Open the database. Room applies any registered migration on the way in. */
    Open,

    /** Show the consent screen, then wipe — a debug build with no migration to offer (ADR-0007). */
    Consent,
 ...
 * migrations, because a build takes migrations **or** the fallback and never both (ADR-0023's
 * Phase 4b amendment): a debug build has no migration registered at all, so a path that exists on
 * paper is not one that build can walk. Asking about migrations first would send a debug build
 * through `Open` and let Room empty the file with no consent screen — the exact thing ADR-0007
 * forbids.
 *
...
fun schemaGateDecision(
    ...
    steps: List<Pair<Int, Int>> = BUNNY_MIGRATION_STEPS,
...
/** [BUNNY_MIGRATIONS] as the integer pairs [schemaGateDecision] reasons over. */
val BUNNY_MIGRATION_STEPS: List<Pair<Int, Int>> =
    BUNNY_MIGRATIONS.map { migration -> migration.startVersion to migration.endVersion }
