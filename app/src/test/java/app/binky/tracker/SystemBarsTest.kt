...
class SystemBarsTest {
    ...
    fun `the theme draws its own system bars and takes their colour from the scrim`() {
        ...
        assertTrue(
            "Theme.Binky must set windowDrawsSystemBarBackgrounds — without it the two colours " +
        ...
        for (bar in listOf("statusBarColor", "navigationBarColor")) {
            assertTrue(
                "Theme.Binky must set android:$bar to @color/system_bar_scrim. A literal colour " +
