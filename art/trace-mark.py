...
Paste them into `rabbit.py`'s MARK and EYE. The paste is deliberate rather than automated:
`rabbit.py` is the single definition of the mark, and a definition a script rewrites on every
...
def insideness(blur=BLUR):
    ...
    if blur:
        ...
        # "torn paper" edge along the rabbit's base at 512px. Blurring the field averages the
    ...
    W, H = im.size
    px = im.load()
    ax = tuple(CREAM[i] - ROSE[i] for i in range(3))
    den = sum(c * c for c in ax)
    f = [[0.0] * H for _ in range(W)]
    for y in range(H):
        for x in range(W):
            c = px[x, y]
            t = sum((c[i] - ROSE[i]) * ax[i] for i in range(3)) / den
...
def main():
    ...
    # Normalise for rabbit.py: every subpath stored clockwise on screen (positive shoelace
