...
indistinguishable from a rabbit in trouble. We therefore only warn about a rabbit's health when an
...
Chasing the owner for fresh observations happens only while a **Watch** is active on that rabbit, and
that nagging is framed as a prompt about the owner's checking, not as a claim about the rabbit.
