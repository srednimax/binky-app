...
- [ ] **Phase 1** — Data layer, rabbits, photos
...
- Rabbit list / add / edit / archive / delete, per ADR-0004. Avatar from camera or album.
- Per-rabbit photo gallery: lazy grid, full-screen pager, captions.
- Missing-media placeholder, required by ADR-0005.

**Gate:** two rabbits with avatars survive a restart; deleting a rabbit with records requires two
...
- First-run setup: add first rabbit (skippable) → backup scope → reminders opt-in (skippable), per ADR-0006.
...
Care reminders depend only on a rabbit existing, and use the simpler mechanism. Building them first
...
- Watch: opt-in per rabbit; only while active does the app chase for fresh observations (ADR-0001).
- Battery-optimisation exemption requested here, at the point something is first scheduled.

**Gate:** a reminder set for +2 minutes fires while backgrounded and still fires after a reboot.

## Phase 5 — Vet, medications, documents, dose reminders

- Vets directory; visits linked to a rabbit and optionally a vet. A weight recorded on a visit also writes
...
- Documents via the ML Kit scanner, attached to a rabbit and optionally a visit; reorder, delete, view.
