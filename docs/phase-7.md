...
| Chart empty states | `8a` / `8b` | ✅ 2026-08-11. Four states, not one. **The app's only artwork**: a 240×96 grid with one ring, drawn on the two single-point states and deliberately not on the other two — an empty grid says nothing the sentence has not, and starts to read as a chart that failed to load. The ring is the plot's own marker at the plot's own size |
| Watch expiry prompt | `8c` / `8d` / `8e` | ✅ 2026-08-11. `8d` is the case with no live flag, *"where silence must not read as good news"* — ADR-0001 drawn, and it already shipped correct. The redraw is `BinkyDialog`, the nested flag, and **a quiet *Close it***. The seed cannot show `8c` — it pairs the flag with the *running* watch on purpose |
