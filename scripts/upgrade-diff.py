#!/usr/bin/env python3
"""Diff two Binky backups and prove an upgrade lost nothing.
...
`adb shell run-as <pkg>` is refused and `bunny.db` cannot be pulled. The app's
...
def extract_db(zip_path, into):
    """Pull database/bunny.db out of a backup, WAL included if one is there.
    ...
    back as present from a bunny.db pulled without its sibling.
    """
    try:
    ...
    with archive:
        ...
        if DB_ENTRY not in names:
            fail(f"{zip_path} has no {DB_ENTRY} — is it a Binky backup?")

        target = os.path.join(into, "bunny.db")
